 ...
 */
angular.module("manager_itemlist",[])
    .controller("mitemlistctrl",["$scope","$location","$http",function($scope,$location,$http){
        $scope.itemsList = {
            "w_01":{
                "name" : "宠物寄养",
                "price" : 100,
        ...
        $http.get("http://127.0.0.1:8080/shopping/out")
        ...
        $scope.sure = function(a){
            var isDel = window.confirm("确认处理？");
            if(isDel){
                $http.get("http://127.0.0.1:8080/mangeritem/update?id="+ a.id+"&userid="+ a.userid+"&itemid="+ a.itemid+"&itemnum="+ a.itemnum+"&itime="+ a.itime+"&itemstate=已处理")
                    .success(function(data){
                        var index = getIndex(a.id);
